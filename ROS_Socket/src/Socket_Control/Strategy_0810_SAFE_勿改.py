...
image_x = [1827.0, 1464.0, 649.5, 1003.5, 1363.0, 621, 281, 1371, 1202, 1020, 846.5, 1372.5, 807.5, 239, 1924, 150.5, 1832, 1875.5, 210] #from0~9, without 1 影像中的X值
arm_x = [25.5, 12.5, -10.5, -0.5, 9.5, -11.5, -23.5, 10, 5, 0, -5, 10, -6, -25, 29, -28, 26, 28, -26] #from0~9, without 1 手臂的X值
image_y = [444.5, 652.5, 617, 831, 973.5, 1008.5, 1143.5, 378.5, 1089.5, 406, 1118.5, 1150, 443, 1056, 766, 734.5, 1102, 429.5, 429] #from0~9, without 1 影像中的Y值
arm_y = [55, 48, 49, 43, 39, 38, 33, 56, 36, 55, 35, 34, 54, 36, 45, 46, 34, 56, 56] #from0~9, without 1 手臂的Y值
# image_x = [1565, 554.5, 1002.5, 1313, 194, 227, 1358.75, 586.5, 1311] #from0~9, without 1 影像中的X值
# arm_x = [22, -14, 2, 13, -27, -26, 27, -13, 13] #from0~9, without 1 手臂的X值
# image_y = [412.5, 535.5, 696.5, 856, 372.5, 949, 966.5, 865.5, 583] #from0~9, without 1 影像中的Y值
# arm_y = [52.8, 48.8, 42.8, 36.8, 54.8, 33.8, 32.8, 36.8, 46.8] #from0~9, without 1 手臂的Y值
    ...
    if Arm_state_flag == Arm_status.Idle and Sent_data_flag == 1:
        ...
        for case in switch(action): #傳送指令給socket選擇手臂動作
            if case(0):            #---------------------------拍照位置---------------------------
                ...
                ArmTask.Arm_Mode(2,1,0,90,2)#action,ra,grip,vel,both
            ...
            if case(2):                #---------------------------------------------
                ...
                pos.yaw = VT 
                ArmTask.point_data(pos.x,pos.y,pos.z,pos.pitch,pos.roll,pos.yaw)
                ArmTask.Arm_Mode(3,1,0,60,2)#action,ra,grip,vel,both
                print('x: ',pos.x,' y: ',pos.y,' z: ',pos.z,' pitch: ',pos.pitch,' roll: ',pos.roll,' yaw: ',pos.yaw)
                action = 3
                break
            if case(3):                #------------------------擊球起始點---------------------
                ...
                pos.yaw = VT 
                ArmTask.point_data(pos.x,pos.y,pos.z,pos.pitch,pos.roll,pos.yaw)
                ArmTask.Arm_Mode(3,1,0,2,2)#action,ra,grip,vel,both
                print('x: ',pos.x,' y: ',pos.y,' z: ',pos.z,' pitch: ',pos.pitch,' roll: ',pos.roll,' yaw: ',pos.yaw)
                action = 4
                break
            if case(4):                  #------------------------擊球-----------------------
            ...
            if case(5):                  #-----------------------擊球結束點上方--------------------------------
                ...
                pos.yaw = VT 
                ArmTask.point_data(pos.x,pos.y,pos.z,pos.pitch,pos.roll,pos.yaw)
                ArmTask.Arm_Mode(3,1,0,50,2)#action,ra,grip,vel,both
                print('x: ',pos.x,' y: ',pos.y,' z: ',pos.z,' pitch: ',pos.pitch,' roll: ',pos.roll,' yaw: ',pos.yaw)
                action = 6
                break
            if case(6):                   #--------------------拍照位置----------------------------------------
