...
def curve():
    ...
    M[0] = cfx(pic_M[0])
    if M[0] > -0.1 :
        M[0] = M[0] - ( M[0]-(-0.1)) * ( 1.91 / 29 )  
    if M[0] < -0.1 :
        M[0] = M[0] + (-0.1 - M[0] ) * ( 1.61 / 29 )  
    M[1] = cfy(pic_M[1])
    if M[1] > 43.3 :
        M[1] = M[1] + 1.3 #( M[1]- 43.3 ) * ( 1.5 / 12.5)   
    if M[1] < 43.3 :
        M[1] = M[1] + 2.68 #( 43.3 - M[1] ) *  (2.26 / 10.5)
        

    C[0] = cfx(pic_C[0])
    if C[0] > -0.1 :
        C[0] = C[0] - ( C[0]-(-0.1)) * ( 1.91 / 29 ) 
    if C[0] < -0.1 :
    ...
    if Arm_state_flag == Arm_status.Idle and Sent_data_flag == 1:
        ...
        for case in switch(action): #傳送指令給socket選擇手臂動作
            ...
            if case(3):                #------------------------擊球起始點---------------------
                ...
                pos.yaw = VT 
                ArmTask.point_data(pos.x,pos.y,pos.z,pos.pitch,pos.roll,pos.yaw)
                ArmTask.Arm_Mode(3,1,0,10,2)#action,ra,grip,vel,both
                print('x: ',pos.x,' y: ',pos.y,' z: ',pos.z,' pitch: ',pos.pitch,' roll: ',pos.roll,' yaw: ',pos.yaw)
                action = 4
                break
            if case(4):                  #------------------------擊球-----------------------
